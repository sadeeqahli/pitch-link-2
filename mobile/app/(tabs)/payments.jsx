...
export default function Payments() {
  ...
  const handleSavePDF = async () => {
    try {
      // Create a text summary of the payment data
      let reportText = "Payment Report\n\n";
      
      if (financialData) {
        reportText += `Today: ₦${financialData.earnings.today.toLocaleString()}\n`;
        reportText += `This Week: ₦${financialData.earnings.weekly.toLocaleString()}\n`;
        reportText += `This Month: ₦${financialData.earnings.monthly.toLocaleString()}\n\n`;
        
        reportText += "Recent Transactions:\n";
        financialData.recentActivity.forEach((transaction, index) => {
          reportText += `${index + 1}. ${transaction.player_name} - ${transaction.pitch_name}\n`;
          reportText += `   Amount: ₦${transaction.total_amount.toLocaleString()}\n`;
          reportText += `   Status: ${transaction.payment_status}\n`;
          reportText += `   Date: ${new Date(transaction.created_at).toLocaleDateString()}\n\n`;
        });
      } else {
        reportText += "No payment data available.\n";
      }
      
      reportText += "Generated by Pitch Link";
      
      // In a real app, this would generate and save a PDF
      // For now, we'll show an alert with the report content
      Alert.alert(
        "Save Payments Report",
        "In a real app, this would generate and save a PDF report of your payments data. Here's a preview of the content that would be included:\n\n" + reportText,
        [{ text: "OK" }]
      );
    } catch (error) {
      console.error("Save PDF error:", error);
      Alert.alert("Error", "Unable to save report: " + error.message);
    }
  };
  ...
  const handleShare = async () => {
    try {
      // Create a text summary of the payment data
      let shareText = "Payment Report\n\n";
      
      if (financialData) {
        shareText += `Today: ₦${financialData.earnings.today.toLocaleString()}\n`;
        shareText += `This Week: ₦${financialData.earnings.weekly.toLocaleString()}\n`;
        shareText += `This Month: ₦${financialData.earnings.monthly.toLocaleString()}\n\n`;
        
        shareText += "Recent Transactions:\n";
        financialData.recentActivity.forEach((transaction, index) => {
          shareText += `${index + 1}. ${transaction.player_name} - ${transaction.pitch_name}\n`;
          shareText += `   Amount: ₦${transaction.total_amount.toLocaleString()}\n`;
          shareText += `   Status: ${transaction.payment_status}\n`;
          shareText += `   Date: ${new Date(transaction.created_at).toLocaleDateString()}\n\n`;
        });
      } else {
        shareText += "No payment data available.\n";
      }
      
      shareText += "Generated by Pitch Link";
      
      // For sharing text content, pass the text as the first parameter
      await Sharing.shareAsync(shareText, {
        dialogTitle: "Share Payments Report",
        mimeType: "text/plain",
      });
    } catch (error) {
